...
// ── FAILSAFE: always remove loader after 4s no matter what ──
...
gsap.set('.loader-label', { opacity: 0, y: 10 });
...
document.querySelectorAll('a, button, .menu-item, .gallery-card').forEach(el => {
    el.addEventListener('mouseenter', () => cursor && cursor.classList.add('hover-active'));
    el.addEventListener('mouseleave', () => cursor && cursor.classList.remove('hover-active'));
});
...
function initScrollAnimations() {
    ...
    gsap.to(gsap.utils.toArray('.m-word'), {
        scrollTrigger: { trigger: ".manifesto", start: "top 65%" },
        y: 0, opacity: 1, duration: 1.0, stagger: 0.07, ease: "expo.out",
    });
    ...
    cards.forEach((card, index) => {
        ...
        gsap.to(card, {
            scale: 0.92, filter: "brightness(0.75)", ease: "none",
            scrollTrigger: {
                trigger: cards[index + 1], start: "top 80%", end: "top 15%", scrub: true
            }
        });
    });
